...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $insert_sql = "INSERT INTO shopping_cart (user_id, product_id, quantity, price, total_price, total_mrp, money_saved, location, color, created_date)
                       VALUES ($user_id, $product_id, $quantity, $price, $total_price, $total_mrp, $moneySaved, '$shippingAddress', '$colorOption, NOW())";

        if ($conn->query($insert_sql) === TRUE) {
            // Redirect to a confirmation page
            header("Location: cart.php?product_id=$product_id&quantity=$quantity&colorOption=$colorOption&totalListPrice=$totalListPrice&totalMRPPrice=$totalMRPPrice&moneySaved=$moneySaved");
            exit;
        } else {
